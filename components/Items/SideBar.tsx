...
function SideBar() {
  ...
  const showSmallMenu =
    router.pathname.toString().split("/").length >= 3 || menuIndex.index == 4
      ? true
      : false;
  ...
  const compareDateIsGreat = new Date(data?.subscribe?.endAt) > new Date(Date.now());
  ...
  console.log("data");
   ...
   if(data?.subscribe == null || !data || !compareDateIsGreat  ){
    
    if(data.error == null){
      globalPayment.onOpen()
  ...
  return (
    ...
    >
      ...
      <div className="min-h-[100px] flex items-center justify-start space-x-4 border-b-[1px]  text-xl font-bold  border-white border-opacity-20" >
        ...
        <div className={`${showSmallMenu ? "hidden" : "block"}`}>
          <img
            className="h-[30px] ml-[32px] mt-2 hidden xl:block  "
            src="/images/logo-payme-complet.png"
          />
          <img
            className="h-[30px] ml-[21px] mt-2   xl:hidden  "
            src="/images/logo-payme.png"
          />
        </div>
      </div>
        ...
        <MenuItem
          ...
          icon={
            <RiDashboardFill
              className={`${
                showSmallMenu
                  ? "absolute    left-[30px] w-10 h-10 mr-6  "
                  : "absolute   left-[30px] w-10 h-10 mr-6 xl:static"
              }`}
            />
          }
          name={"Accueil"}
        ...
        <MenuItem
          ...
          icon={
            <BsFillCreditCardFill
              className={`${
                showSmallMenu
                  ? "absolute  left-[30px] w-9 h-9 mr-6  "
                  : "absolute  left-[30px] w-9 h-9 mr-6 xl:static"
              }`}
            />
          }
          name={"Finances"}
          ...
          icon={
            <svg
              className={`${
                showSmallMenu
                  ? "absolute  left-[30px] w-9 h-9 mr-6  "
                  : "absolute  left-[26px] w-9 h-9 mr-6 xl:static"
              }`}
              width="24"
              height="24"
              viewBox="0 0 24 24"
...
function MenuItem({
  ...
  return (
    <div
      ...
      } flex items-center font-semibold    hover:bg-opacity-10 cursor-pointer bg-opacity-5  text-[19px] px-10 ${className}`}
    >
      {icon}{" "}
      <p className={`${isSmallMenu ? "hidden" : "hidden xl:block"} `}>{name}</p>
    </div>
  );
}
